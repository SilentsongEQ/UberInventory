        ...
        local function UberInventory_CopyItems( str, cmdline )
            ...
            for key, record in pairs( UBI_Items_Work ) do
                ...
                if ( record.bag_count ) then
                    record.total = record.bag_count + record.bank_count + record.mailbox_count + record.equip_count;
                end;
                ...
                if ( not cmdline ) then
                    ...
                    if ( ( UBI_FILTER_LOCATIONS == index and record.bag_count == 0 ) or
                         ...
                         ( UBI_FILTER_LOCATIONS == index + 3 and record.equip_count == 0 ) ) then
                        include_item = false;
                    end;
...
-- Add sell prices to items in your bag (only used if not already at a merchant)
    function UberInventory_AddItemInfo( tooltip )
        ...
        local function AddLocationInfo( tooltip, bag, bank, mailbox, equipped, itemid )
                ...
                if ( UBI_Options["show_item_count"] ) then
                    ...
                    if ( UBI_TooltipItem ) then
                        ...
                        if ( UBI_TooltipItem["bag_count"] and ( UBI_LocationList[UBI_FILTER_LOCATIONS].type == "current" or
                            ...
                            AddLocationInfo( tooltip,
											 ...
											 itemId );
                            totalCount = totalCount + UBI_TooltipItem["bag_count"] + UBI_TooltipItem["bank_count"] + UBI_TooltipItem["mailbox_count"] + UBI_TooltipItem["equip_count"];
                            spacerAdded = true;
                        end;
                    elseif ( UBI_Items[itemId] ) then
                        AddLocationInfo( tooltip,
										 ...
										 itemId );
                        totalCount = totalCount + UBI_Items[itemId]["bag_count"] + UBI_Items[itemId]["bank_count"] + UBI_Items[itemId]["mailbox_count"] + UBI_Items[itemId]["equip_count"];
                        spacerAdded = true;
                    end;
...
-- Get item information and store it
    function UberInventory_Item( bagID, slotID, location )
        ...
        if ( itemLink ) then
            ...
            local totalCount = bagCount + bankCount + mailCount + equipCount;
...
-- Handle Bank items
    function UberInventory_Save_Bank()
        ...
        if ( UBI_BANK_OPEN ) then
            ...
            -- Traverse bank tabs - Six bank tabs, each with 98 slots = 588 slots
			for characterBankTabID = Enum.BagIndex.CharacterBankTab_1, Enum.BagIndex.CharacterBankTab_6 do
                if ( C_Container.GetContainerNumSlots( characterBankTabID ) ) then
                    slotCount = slotCount + C_Container.GetContainerNumSlots( characterBankTabID );
                    freeCount = freeCount + C_Container.GetContainerNumFreeSlots( characterBankTabID );
                    for slotID = 1, C_Container.GetContainerNumSlots( characterBankTabID ) do
                        UberInventory_Item( characterBankTabID, slotID, "bank" );
                    end;
                end;
            end;
...
-- Cleanup inventory
    function UberInventory_Cleanup()
        ...
        for key, record in pairs( UBI_Items ) do
            if ( record.name ) then
                if ( record.bag_count + record.bank_count + record.mailbox_count + record.equip_count == 0 ) then
                    -- Remove the item
                    UBI_Items[key] = nil;
                end;
            else
...
-- Handle events sent to the addon
    function UberInventory_OnEvent( self, event, ... )
        ...
        if ( event == "BAG_UPDATE" and arg1 >= -2 ) then
            ...
            if ( ( arg1 == BANK_CONTAINER) or ( arg1 >= 5 and arg1 <= 11 ) ) then
				UberInventory_Save_Bank();
			end;
	...
	function UberInventory_UpgradeMessage()
		StaticPopupDialogs["UberInventory_UpgradeMessage"] = {
			text = C_GOLD..UBI_NAME..C_GOLD.." - "..UBI_VERSION.." Information"..C_CLOSE..
				   ...
				   "\n\n** "..C_RED.."!! Visit the bank on each character !! **"..C_CLOSE..
				   "\n\n   > Significant changes to how banks work with this patch will require a visit to the bank on each character."..
				   "\n\n   > Void storage and Reagent banks have been removed."..
				   C_BLUE.."\n\nApologies for the delay in providing this update"..C_CLOSE..
				   "\n\nSilentsongEQ"..
				   C_CLOSE.."\n\n",
			button1 = "OK",
			timeout = 0,
